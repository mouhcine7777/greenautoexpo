...
const ProgramSection = () => {
  ...
  return (
    <section className="bg-gradient-to-b from-white to-[#6BBEA3]/10 py-24 font-montserrat">
      <div className="container mx-auto px-4">
        ...
        >
          <h2 className="text-4xl lg:text-5xl font-bold text-[#252F46] mb-6">
        ...
        <div className="flex justify-center space-x-4 mb-12 overflow-x-auto pb-4">
          {programs.map((program) => (
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              className={`relative px-6 py-3 rounded-xl flex flex-col items-center min-w-[120px]
                ${activeDay === program.day 
                  ? 'bg-[#252F46] text-white' 
                  : 'bg-white text-[#252F46] hover:bg-[#252F46]/5'}`}
              onClick={() => setActiveDay(program.day)}
            >
              <span className="text-sm">Jour {program.day}</span>
          ...
          >
            {programs.find(p => p.day === activeDay)?.events.map((event, index) => (
              <motion.div
                ...
                className="bg-white rounded-2xl p-6 mb-4 shadow-lg shadow-[#6BBEA3]/5 
                         hover:shadow-xl hover:shadow-[#6BBEA3]/10 transition-all"
              >
                <div className="flex items-start gap-4">
                  <div className="bg-[#6BBEA3]/10 rounded-xl p-3 text-center min-w-[100px]">
                    <Clock className="w-6 h-6 text-[#6BBEA3] mx-auto mb-1" />
                    <span className="text-[#252F46] font-semibold">{event.time}</span>
                  </div>
                  
                  <div className="flex-1">
                    <h3 className="text-xl font-semibold text-[#252F46] mb-2">
                      {event.title}
                    </h3>
                    <div className="flex flex-wrap gap-4 text-gray-600">
                      ...
                      <div className="flex items-center gap-2">
                        <Star className="w-4 h-4 text-[#6BBEA3]" />
                        {event.type}
                      </div>
                    </div>
                  </div>
